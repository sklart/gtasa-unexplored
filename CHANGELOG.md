...
Финальный выпуск на основе `0.5.0-rc4`.

- POI-маркеры масштабируются по высоте с сохранением пропорций.
- Выбор markers использует единую identity: первый tap выделяет объект,
  повторный tap по той же метке открывает Details.
...
- POI используют высоту как базовый размер, сохраняя пропорции изображения и
  не перекрывая collectibles на дальнем масштабе.
- Выбор marker использует единую identity «тип + индекс»: первый tap выделяет,
  повторный tap по той же метке открывает Details, включая POI.
- Подробная матрица прогресса перенесена из sidebar в отдельное окно с
  подписанными иконками типов, итогами по регионам и touch/gamepad-открытием.

## 0.5.0-rc3

> Release candidate: не проверен на физическом Nintendo Switch.

- Устранены пересечения выбранной метки, progress matrix и блока управления на
  правой панели; матрица получила подписанные колонки с пиктограммами типов.
- Route Mode показывает лимит и текущую точку, а не неактуальную сортировку;
  L/R в этом режиме не изменяют состояние сортировки.
- POI-пины уменьшены на далёком масштабе, а курсор рендерится непрозрачно на
  дробных координатах без визуального раздвоения.
- Filters, Object List и карточки закрываются touch-крестиком с зоной 48×48 px;
  касание крестика не передаётся карте или жестам.
