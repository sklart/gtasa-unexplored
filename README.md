...
- Фильтры пяти категорий collectibles и POI: общий переключатель и отдельные
  категории сюжетных мест, достопримечательностей, природы, тайн и заведений;
  состояние сохраняется в `config.ini`.
...
**↑/↓** прокручивают текст, **L3/R3** переключают включённые объекты по всей
карте. Для
...
Статус **Проверено** / `Verified` означает точную координату конкретного места;
**Ориентировочно** / `Approximate` — удобную точку для крупного или протяжённого
объекта без фиктивной высоты. Жёлтый pin означает ориентировочную, а не
телепорт-координату. Нажмите A или
коснитесь pin повторно, чтобы открыть карточку с фотографией.

В Filters строка **POI: все категории** управляет всеми POI, а следующие пять
...
режим «только избранное». В Route Mode L/R отключены, а заголовок показывает
текущую точку маршрута. На карте **ZR + Y** переключает избранное у текущей
выбранной метки. Все эти параметры хранятся локально в `config.ini`.

Filters, список и карточки закрываются прежними кнопками контроллера, а также
видимым крестиком в правом верхнем углу; его touch-зона увеличена до 48×48 px.
На боковой панели отображается общий прогресс четырёх регионов. Кнопка
**«Таблица»** рядом с ним (либо **ZR + −**) открывает подробный кэшированный
прогресс «регион × тип collectible»; **B** или крестик закрывают окно.
...
- **L / R** — масштаб;
- **↑ / ↓** — предыдущая / следующая подложка; выбранная карта сохраняется в `config.ini` как `map_id`;
- **← / →** — предыдущий / следующий save slot;
- **L3 / R3** — предыдущий / следующий включённый collectible по всей карте;
- **ZL + L3 / R3** — предыдущая / следующая метка в группе перекрытия;
- **ZR + R3 / touch двумя пальцами** — скрыть/показать правую панель;
- **ZR + A** — выбрать ближайший ненайденный collectible, подходящий под активные фильтры;
...
- **A** на строке **«Режим объектов»** — переключить **Не найдено → Найдено → Все**;
- **ZL** — RU/EN (только в окне Filters);
- **X** — закрыть.

Пункт **«Все категории»** в секции POI включает и выключает все маркеры достопримечательностей,
а пять строк ниже управляют их категориями независимо от коллекционных объектов. Значки коллекционных объектов — встроенные PNG с лёгкой тенью; найденные
